...
# (source_id, 模块名, 类名, 显示名, 支持代理, 推荐场景)
_SOURCES = [
    ("eztv", "rss_source_eztv", "EZTVRSSSource", "EZTV", True, "us_tv"),
    ("yts", "rss_source_yts", "YTSRSSSource", "YTS", True, "movie"),
    ("prowlarr", "rss_source_prowlarr", "ProwlarrRSSSource", "Prowlarr", False, "us_tv"),
]
...
def register(ctx):
    ...
    loaded = []
    for source_id, module_name, class_name, name, supports_proxy, recommended in _SOURCES:
        try:
            source_class = _load_source_class(ctx, module_name, class_name)
            capabilities = ["rss", "download_url", f"recommended_{recommended}"]
            if source_id == "prowlarr":
                capabilities.append("recommended_movie")
            ctx.register_rss_source_provider(
                ...
                description=f"{name} 影视订阅源",
            )
            loaded.append(name)
        except Exception as exc:
            ctx.logger.error(f"[rss-tv-movie] {name} 注册失败: {exc}")

    if loaded:
        ctx.logger.info(f"影视 RSS 源已加载：{'/'.join(loaded)}")
    else:
        ctx.logger.error("影视 RSS 源：没有任何源注册成功")
